...
struct Data * getData(char * line)
{
    ...
    if (data != NULL)
    {
        ...
        //Allocate data for the string
        data->tradingName = (char*) malloc(strlen(s)+1);
        if (data->tradingName)
        {
            // store trading name
            strcpy(data->tradingName, s);
        }
        // free s
        free(s);
...
void GetDelimType(const char *line, char *delim, int *i) {// get the substring
    if (line[(*i)] == '"'){
        (*delim) = '"';
        (*i) +=1;
    }
...
{
    // print the data :)
    fprintf(fp, "%s --> Census year: %d || Block ID: %d || Property ID: %d || Base property ID: %d || CLUE small area: %s || Industry (ANZSIC4) code: %d || Industry (ANZSIC4) description: %s || x coordinate: %.4lf || y coordinate: %.4lf || Location: %s \n\n", data->tradingName, data->censusYear, data->blockID, data->propertyID, data->basePropertyID, data->CLUESmallArea, data->industryCode, data->industryDescription, data->XCoordinate, data->YCoordinate, data->location );
}
